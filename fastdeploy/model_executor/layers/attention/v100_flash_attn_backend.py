...
class V100FlashAttentionBackend(AttentionBackend):
    ...
    def _apply_rotary_emb(
        ...
        Note: For V100, RoPE is typically handled by the model's QKV projection
        or needs position_ids for proper indexing. This simplified implementation
        skips RoPE application when shape mismatch is detected, as the fused
        kernels (gqa_rope_write_cache) that properly handle RoPE require SM80+.

        Args:
            q: Query tensor [num_tokens, num_heads, head_dim]
            k: Key tensor [num_tokens, kv_num_heads, head_dim]
            rotary_embs: Rotary embeddings tensor
            use_neox_rotary_style: Whether to use neox rotary style

        Returns:
            q_rotated: Query tensor (possibly with RoPE applied)
            k_rotated: Key tensor (possibly with RoPE applied)
        """
        # Skip RoPE if not provided
        if rotary_embs is None:
            return q, k

        # For V100, we skip the manual RoPE implementation because:
        # 1. The fused gqa_rope_write_cache kernel handles RoPE properly but requires SM80+
        # 2. Manual RoPE needs position_ids for correct indexing in batched scenarios
        # 3. Many models apply RoPE in the QKV projection layer already
        #
        # This may affect model accuracy for models that rely on attention-layer RoPE.
        # For production V100 support, consider:
        # - Using paddle.nn.functional.fused_rotary_position_encoding
        # - Implementing proper position_id based indexing
        logger.warning(
            "V100FlashAttentionBackend: Skipping RoPE application. "
            "RoPE is typically handled by fused kernels (SM80+) or model's QKV projection."
        )
        return q, k
